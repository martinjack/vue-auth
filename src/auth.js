...
function _isAccess(role, key) {
  if (__auth.$vm.state.authenticated === true) {
    if (role) {
      return __utils.compare(
        role,
        __utils.getProperty(
          __auth.$vm.state.data || {},
          key || __auth.options.rolesKey
        )
      );
    }
...
function _parseResponseIntercept(res, req) {
  ...
  if (token) {
    __token.set.call(
      ...
      null,
      token,
      __token.get.call(__auth, __auth.options.staySignedInKey) ? false : true
    );
  }
}
...
function _processAuthenticated(cb) {
  if (__auth.$vm.state.authenticated === null && __token.get.call(__auth)) {
...
function _processTransitionEach(transition, routeAuth, cb) {
  var authRedirect = (routeAuth || "").redirect || __auth.options.authRedirect,
    forbiddenRedirect =
      ...
      (routeAuth || "").redirect ||
      __auth.options.forbiddenRedirect,
    notFoundRedirect =
      (routeAuth || "").notFoundRedirect ||
      (routeAuth || "").redirect ||
      __auth.options.notFoundRedirect,
  ...
  if (
    ...
    if (!__auth.check()) {
      ...
      !__utils.compare(
        routeAuth,
        __utils.getProperty(__auth.$vm.state.data || {}, rolesKey)
      )
    ) {
...
function _processImpersonate(defaultToken, redirect) {
  __token.set.call(
    __auth,
    __auth.options.tokenImpersonateKey,
    __auth.token(),
    __token.get.call(__auth, __auth.options.staySignedInKey) ? false : true
  );
  __token.set.call(
    __auth,
    __auth.options.tokenDefaultKey,
    defaultToken,
    __token.get.call(__auth, __auth.options.staySignedInKey) ? false : true
  );
...
function _initInterceptors() {
  __auth.drivers.http.interceptor.call(
    __auth,
    _parseRequestIntercept,
    _parseResponseIntercept
  );

  __auth.drivers.router.beforeEach.call(
    __auth,
...
Auth.prototype.impersonating = function () {
  var impersonating = __token.get.call(
    __auth,
    __auth.options.tokenImpersonateKey
  )
    ? true
    ...
    if (token === null) {
      ...
      expires =
        expires === true || expires === false
          ? expires
          : __token.get.call(__auth, __auth.options.staySignedInKey)
          ? false
          : true;
  ...
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (
        ...
        await __auth.fetch({ redirect: data.redirect }).then(resolve, reject);
      } else {
        await _processFetch(await _parseUserResponseData(res), data.redirect);
        resolve(res);
      }
